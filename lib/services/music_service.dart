...
class MusicService {
  /// 使用kanade_audio_plugin实现媒体库访问
  static final plugin.KanadeAudioPlugin _audioPlugin =
      plugin.KanadeAudioPlugin();
      ...
      for (int i = 0; i < entries.length; i += batchSize) {
        ...
        for (final result in results) {
          ...
          if (albumArt != null && albumArt.isNotEmpty) {
            // updatedSongs[index] = updatedSongs[index].copyWith(
            //   albumArt: albumArt,
            // );
            if (updatedSongs[index].albumId != null) {
              CoverCacheManager.instance.setCover(
                updatedSongs[index].albumId!,
                albumArt,
              );
            }
            hasUpdates = true;
